...
const TOOLTIPS = {
  sentiment:
    "Score from -1.0 (very bearish) to +1.0 (very bullish). The Portfolio Manager produces this after weighing all three personas' stances and convictions.",
  confidence:
    'How certain the committee is in their call. A split debate lowers this — a unanimous 3-0 vote at high conviction produces higher confidence than a 2-1 split.',
  committee:
    "Three AI personas debate the headline sequentially. Each reads the previous persona's full opinion before responding, producing genuine disagreement rather than independent monologues.",
  conviction:
    "How strongly this persona believes their own stance. A high-conviction dissenter (0.8+) can pull the Portfolio Manager's final confidence down even if outvoted 2-1.",
  bullish: 'Expects the stock price to rise. The bull thrusts its horns upward — prices going up.',
  bearish:
    'Expects the stock price to fall. The bear swipes its paws downward — prices going down.',
  neutral:
    'No strong directional view. Acts as a dissenting voice against whichever side has the majority, lowering overall confidence.',
  consensus:
    "The Portfolio Manager's final synthesis — written after weighing all three debate opinions and their conviction scores. This is the reasoning behind the BUY / SELL / HOLD decision.",
  action:
    'The final trade decision. BUY = purchase shares, SELL = sell shares, HOLD = do nothing. Only fires a real order if sentiment and confidence both clear the configured thresholds.',
  marketOrder:
    'Executes immediately at the current market price. Chosen over limit orders because news-driven trades prioritise speed over price precision.',
  paperTrading:
    'Simulated trading with real market data but no real money. Used to validate strategy before ever risking live capital.',
};
...
function StanceBadge({ stance }: { stance: string }) {
  const colour =
    stance === 'BULLISH'
      ? 'text-positive bg-positive-soft border-positive-border'
      : stance === 'BEARISH'
        ? 'text-negative bg-negative-soft border-negative-border'
  ...
  return (
    <span
      className={`group relative inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[10px] font-bold ${colour}`}
    >
      {stance}
...
export default function AgentMonologue({
      ...
      <div className="glass-panel flex h-full min-h-[280px] items-center justify-center rounded-2xl p-10 xl:min-h-0">
        <div className="max-w-xs text-center">
          ...
          <p className="mt-1.5 text-sm text-muted">
            Select a signal to inspect the agent&apos;s reasoning trace.
          </p>
        </div>
      </div>
    );
  ...
  const consensusReasoning =
    trade.reasoning ?? decisionTrace.portfolio_manager_decision?.reasoning ?? '';
            ...
            {articleUrl && (
              ...
              >
                <span className="max-w-[140px] truncate">
                  {articleSourceLabel(trade.article_source)}
                </span>
                <ExternalLinkIcon />
              </a>
            )}
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-xl border border-line bg-surface-2 p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span
                className={`font-mono text-sm font-bold ${trade.sentiment_score >= 0 ? 'text-positive' : 'text-negative'}`}
              >
                {trade.sentiment_score >= 0 ? '+' : ''}
                {trade.sentiment_score.toFixed(3)}
              </span>
            </div>
        ...
        {committee.length > 0 && (
          <div className="rounded-xl border border-line bg-surface-2 p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-[10px] text-muted">{summariseVote(committee)}</span>
            </div>
          ...
          <div className="flex items-start gap-3 rounded-xl border border-warning-border bg-warning-soft px-4 py-3.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[11px] leading-relaxed text-warning opacity-80">
                This signal was injected via the Simulate feature. Alpaca orders are always skipped
                for simulated signals, regardless of confidence or sentiment scores.
              </p>
            </div>
          </div>
        )}
        ...
        {trade.order_id && !trade.is_simulated && (
          <div className="flex items-start gap-3 rounded-xl border border-positive-border bg-positive-soft px-4 py-3.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 break-all font-mono text-[11px] text-positive opacity-80">
                {trade.order_id}
              </p>
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-2 p-4">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        <MetricPill
          label="Source"
          value={`${quality.grade ?? 'UNKNOWN'} ${typeof quality.score === 'number' ? quality.score.toFixed(2) : ''}`}
          className={qualityColour(quality.grade)}
        />
      ...
      {blockers.length > 0 && (
        <div className="mt-3 space-y-1.5">
          {blockers.slice(0, 4).map((blocker) => (
            <div
              key={blocker}
              className="rounded-lg border border-line bg-surface px-3 py-2 text-[11px] leading-relaxed text-muted"
            >
              {blocker}
            </div>
          ))}
...
function MetricPill({
  label,
  value,
  className = 'border-line bg-surface text-primary',
}: {
  label: string;
  value: string;
  className?: string;
      ...
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate font-mono text-[9px] text-muted">
            {operation.response_schema ?? operation.kind}
            {model ? ` · ${model}` : ''}
          </p>
        </div>
        <span
...
function PersonaCard({ persona }: { persona: PersonaOpinion }) {
  const meta = PERSONA_META[persona.name] ?? {
    ...
    colour: 'bg-surface-3 text-muted border-line',
  };
  const convictionPct = Math.round(persona.conviction * 100);
  const convictionColour =
    convictionPct >= 70 ? 'bg-accent' : convictionPct >= 40 ? 'bg-amber-500' : 'bg-muted';
  ...
  return (
    <div className="min-w-0 rounded-lg border border-line bg-surface p-3.5">
      ...
      <div className="flex items-center gap-2.5">
        <div
          className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md border text-xs font-bold ${meta.colour}`}
        >
          {meta.initial}
        </div>
      ...
      {(persona.evidence_quality ||
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          ...
          {persona.catalyst_strength && (
            <TinyBadge label={`Catalyst ${persona.catalyst_strength}`} />
          )}
      ...
      {persona.reasoning && persona.reasoning !== persona.view && (
        <p className="mt-1.5 text-[11px] leading-relaxed text-secondary">{persona.reasoning}</p>
      )}
    </div>
  );
}
...
function InfoIcon() {
  return (
    <svg
      className="h-3 w-3 shrink-0 text-muted opacity-60 transition-opacity group-hover:opacity-100"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth={2}
