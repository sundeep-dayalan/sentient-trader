...
function ChartTooltip({
  ...
  return (
    <div className="rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-card)] px-4 py-3 text-xs shadow-2xl backdrop-blur">
      <p className="mb-1 text-[var(--dashboard-subtle)]">
        {label
      ...
      <p className="font-mono text-base font-bold text-[var(--dashboard-text)]">
        {fmt(payload[0].value)}
      </p>
    </div>
  );
...
export default function PnLChart() {
  ...
  const accountLabel =
    account?.accountNumber ?? (account?.id ? `...${account.id.slice(-6)}` : null);
  ...
  return (
    <section className="relative min-h-[360px] overflow-hidden rounded-2xl border border-[var(--dashboard-border)] bg-[var(--dashboard-chart)] p-5 shadow-[var(--dashboard-shadow)] md:min-h-[400px]">
      <div
        ...
        style={{ background: 'var(--dashboard-chart-overlay)' }}
      />
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto]">
          <div className="min-w-0">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-5">
              ...
              <p
                className={`inline-flex items-center gap-2 font-mono text-base font-semibold ${isUp ? 'text-positive' : 'text-negative'}`}
              >
                <svg
            ...
            <label className="inline-flex h-12 items-center gap-2.5 rounded-xl bg-[var(--dashboard-control)] px-5 text-sm font-semibold text-[var(--dashboard-text)] shadow-sm">
              ...
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[var(--dashboard-row)] text-[var(--dashboard-subtle)]">
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M5.23 7.23a.75.75 0 0 1 1.06 0L10 10.94l3.71-3.71a.75.75 0 1 1 1.06 1.06l-4.24 4.24a.75.75 0 0 1-1.06 0L5.23 8.29a.75.75 0 0 1 0-1.06z"
                    clipRule="evenodd"
                  />
                </svg>
              </span>
                ...
                <AreaChart data={data} margin={chartMargin}>
                  <defs>
                    <linearGradient id="paper-portfolio-fill" x1="0" y1="0" x2="0" y2="1">
                      <stop
                        offset="0%"
                        stopColor="var(--dashboard-chart-fill-1)"
                        stopOpacity="0.55"
                      />
                      <stop
                        offset="55%"
                        stopColor="var(--dashboard-chart-fill-2)"
                        stopOpacity="0.22"
                      />
                      <stop
                        offset="100%"
                        stopColor="var(--dashboard-chart-fill-3)"
                        stopOpacity="0"
                      />
                    </linearGradient>
                  </defs>
