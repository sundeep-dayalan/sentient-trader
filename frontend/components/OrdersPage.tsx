...
function bucketFor(status?: string): OrderBucket {
  ...
  if (normalized === "canceled" || normalized === "expired" || normalized === "done_for_day") return "canceled";
  if (normalized === "rejected" || normalized === "failed" || normalized === "suspended") return "rejected";
  return "other";
}
...
export default function OrdersPage() {
  ...
  const selectedOrder = data.orders.find(order => order.id === selectedId) ?? filteredOrders[0] ?? null;
  const selectedCancelOrders = data.orders.filter(order => selectedCancelIds.has(order.id) && canCancel(order.status));
  ...
  const rejectedOrders = data.orders.filter(order => bucketFor(order.status) === "rejected").length;
  ...
  return (
    ...
    <div className="grid min-h-0 flex-1 grid-rows-[auto_minmax(0,1fr)] gap-4">
      <div className="grid shrink-0 grid-cols-2 gap-3 xl:grid-cols-5">
        {[
          { label: "Account Value", value: compactMoney(accountValue), tone: "text-primary", sub: data.account?.status ?? "alpaca" },
          { label: "Buying Power", value: compactMoney(data.account?.buying_power), tone: "text-cyan", sub: data.account?.currency ?? "USD" },
          ...
          { label: "Positions", value: data.positions.length, tone: rejectedOrders > 0 ? "text-negative" : "text-accent", sub: rejectedOrders > 0 ? `${rejectedOrders} rejected` : "open" },
        ].map(card => (
          <div key={card.label} className="metric-card glass-panel-subtle rounded-lg px-4 py-3">
            <p className="truncate text-[10px] font-semibold uppercase text-muted">{card.label}</p>
            <p className={`mt-2 font-mono text-xl font-semibold leading-none ${card.tone}`}>{card.value}</p>
        ...
        <section className="glass-panel flex min-h-[520px] flex-col overflow-hidden rounded-2xl xl:min-h-0">
          <div className="shrink-0 border-b border-[var(--dashboard-divider)] px-5 py-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                Cancel selected {selectedCancelOrders.length > 0 ? `(${selectedCancelOrders.length})` : ""}
              </button>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="font-mono text-sm font-bold text-accent">{order.symbol ?? "-"}</p>
                    <p className="mt-1 text-[10px] text-muted">{dateTime(order.submitted_at ?? order.created_at)}</p>
                  </div>
                  <div className="flex min-w-0 items-start gap-2">
                    <span className={`rounded-md border px-2 py-0.5 text-[10px] font-semibold uppercase ${sideClasses(order.side)}`}>
                      {order.side ?? "-"}
                    </span>
                  </div>
                  <div className="min-w-0">
                    <p className="font-mono text-xs text-secondary">{orderQuantity(order)}</p>
                    <p className="mt-1 text-[10px] text-muted">{order.type ?? order.order_type ?? "order"}</p>
                  </div>
                  <div className="min-w-0">
                    <p className="font-mono text-xs text-secondary">{orderPrice(order)}</p>
                    <p className="mt-1 truncate text-[10px] text-muted">{order.client_order_id ?? order.id}</p>
                  </div>
                  <div className="min-w-0 lg:text-right">
                    <span className={`inline-flex max-w-full truncate rounded-md border px-2 py-0.5 text-[10px] font-semibold uppercase ${statusClasses(order.status)}`}>
            ...
            <div className="modern-scroll min-h-0 flex-1 space-y-4 overflow-y-auto p-4 pr-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 break-all font-mono text-[10px] text-muted">{selectedOrder.id}</p>
                </div>
                <span className={`rounded-md border px-2.5 py-1 text-[11px] font-semibold uppercase ${statusClasses(selectedOrder.status)}`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <DetailRow label="Time In Force" value={selectedOrder.time_in_force?.toUpperCase()} />
              ...
              {selectedOrder.legs && selectedOrder.legs.length > 0 && (
                <div className="rounded-lg border border-line bg-surface-2 p-3">
                  ...
                  <p className="mt-2 text-sm text-secondary">{selectedOrder.legs.length} child order(s)</p>
                </div>
              )}

              <div className="rounded-lg border border-line bg-surface-2 p-3">
                <p className="mb-2 text-[10px] font-semibold uppercase text-muted">Raw Alpaca Payload</p>
      ...
      >
        ...
        >
          <div className="flex items-start justify-between gap-3 border-b border-line pb-3">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-muted">
                This sends cancel requests to Alpaca for open paper orders. Orders already filled or no longer cancelable may be rejected.
              </p>
            </div>
            <button
          ...
          <div className="modern-scroll mt-4 max-h-56 space-y-2 overflow-y-auto pr-2">
            {selectedCancelOrders.map(order => (
              <div key={order.id} className="flex items-center justify-between gap-3 rounded-lg border border-line bg-surface-2 px-3 py-2">
                <div>
                  <p className="font-mono text-sm font-semibold text-accent">{order.symbol ?? "-"}</p>
                  <p className="mt-0.5 text-[10px] text-muted">{orderQuantity(order)} | {order.type ?? order.order_type}</p>
                </div>
                <span className={`rounded-md border px-2 py-0.5 text-[10px] font-semibold uppercase ${statusClasses(order.status)}`}>
                  {order.status}
                </span>
              </div>
            ))}
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {canceling ? "Cancelling..." : `Cancel ${selectedCancelOrders.length} order${selectedCancelOrders.length === 1 ? "" : "s"}`}
            </button>
          </div>
        </div>
      </div>
